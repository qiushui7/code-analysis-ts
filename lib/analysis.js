...
class CodeAnalysis {
  constructor(options) {
    ...
    this.importFrom = {};                                                              // 依赖统计
    this.dependenciesWarn = {};                                                        // 可能未安装的依赖
  ...
  _dealAST(importItems, ast, checker, filePath, projectName, httpRepo, baseLine = 0) {
    ...
    function walk(node) {
      ...
      if (tsCompiler.isImportDeclaration(node) && node.moduleSpecifier && node.moduleSpecifier.text) {
        let temp = node.moduleSpecifier.text
        let alias = that._scanSource.find((item) => item.name === projectName).alias
        let flag = true
        if (alias) {
          alias.forEach((item) => {
        ...
        if (!temp.startsWith('./') && !temp.startsWith('../') && flag) {
          let isScoped = temp.startsWith('@')
          let index = temp.indexOf('/')
          if (index !== -1 && !isScoped) {
            temp=temp.slice(0,index)
          } else if (index !== -1 && isScoped) {
            let secondIndex = temp.indexOf('/', index + 1);
            temp = secondIndex !== -1 ? temp.slice(0, secondIndex) : temp
          }
          if (that.importFrom[projectName]) {
            that.importFrom[projectName].add(temp)
          } else {
            that.importFrom[projectName] = new Set()
            that.importFrom[projectName].add(temp)
          }
        }
      }
  ...
  // 确认使用的依赖已被声明
  _checkImportstatement(scanSource) {
    scanSource.forEach((item)=>{
      if(item.packageFile && item.packageFile !=''){
        try{
          const lockInfo = getJsonContent(item.packageFile);
          ...
          const temp = Object.keys(lockInfo.dependencies).concat(Object.keys(lockInfo.devDependencies));
          if (temp.length > 0) {
            this.importFrom[item.name].forEach((dep) => {
              if (!temp.some((item) => item === dep)) {
                if (!this.dependenciesWarn[item.name]) {
                  this.dependenciesWarn[item.name]=[dep]
                } else {
                  this.dependenciesWarn[item.name].push(dep)
                }
              }
            })
          }
  ...
  analysis() {
    ...
    this._checkImportstatement(this._scanSource)
    // 代码评分
